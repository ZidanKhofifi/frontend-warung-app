...
const LoginPage = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Gagal masuk. Cek akun kembali.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    // Menggunakan bg-slate-100 agar kartu putih terlihat jelas (tidak nyaru)
    <div className="min-h-screen flex items-center justify-center bg-slate-100 p-6 font-sans">
      <div className="w-full max-w-sm">
        ...
        <div className="flex flex-col items-center mb-10">
          ...
          <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] mt-5 bg-white px-5 py-2 rounded-full shadow-sm border border-slate-200">
        ...
        <div className="bg-white p-8 rounded-[2.5rem] shadow-[0_20px_50px_rgba(0,0,0,0.1)] border border-white">
          ...
          <form onSubmit={handleLogin} className="space-y-6">
            ...
            <button
              type="submit"
              disabled={loading}
              className={`w-full py-4 rounded-2xl font-black text-xs uppercase tracking-widest text-white transition-all duration-300 flex items-center justify-center gap-2 mt-2
                ${loading 
          ...
          <div className="mt-10 pt-6 border-t border-slate-100 text-center">
            <p className="text-[11px] text-slate-500 font-bold uppercase tracking-tight">
              Belum punya akun? <Link to="/register" className="text-[#ff9900] font-black ml-1 hover:text-slate-900 transition-colors">Daftar Disini</Link>
            </p>
          </div>
        </div>
      </div>
